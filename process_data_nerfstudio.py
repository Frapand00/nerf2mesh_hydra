...
from utils_studio import install_checks
from process_data.process_data_utils import CAMERA_MODELS


CONSOLE = Console(width=120)
DEFAULT_COLMAP_PATH = Path("colmap_sparse/0")
...
class ProcessImages:
    ...
    def main(self) -> None:  # pylint: disable=R0915
        ...
        if not self.skip_colmap:
            ...
            colmap_model_path = colmap_dir / "colmap_sparse" / "0"
            require_cameras_exist = True
        ...
        if self.use_sfm_depth:
            ...
            image_id_to_depth_path = colmap_utils.create_sfm_depth(
                recon_dir=colmap_dir / "colmap_sparse" / "0",
...
class ProcessVideo:
    ...
    def main(self) -> None:  # pylint: disable=R0915
        ...
        # Save transforms.json
        if (colmap_dir / "colmap_sparse" / "0" / "cameras.bin").exists():
            with CONSOLE.status("[bold yellow]Saving results to transforms.json", spinner="balloon"):
                num_matched_frames = colmap_utils.colmap_to_json(
                    recon_dir=colmap_dir / "colmap_sparse" / "0",
                    output_dir=self.output_dir,
