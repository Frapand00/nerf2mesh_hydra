...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        # MSE loss (stage 0)
        if self.stage == 0:
            outputs = self.model.render(rays_o, rays_d, index=index, staged=False, bg_color=bg_color, perturb=True, cam_near_far=cam_near_far, shading=shading)
        ...
        if self.stage == 1:
            ...
            if self.lambda_lpips > 0:
                loss = loss + self.lambda_lpips * self.criterion_lpips(pred_rgb.view(1, H, W, 3).permute(0,3,1,2).contiguous(), gt_rgb.view(1, H, W, 3).permute(0,3,1,2).contiguous())
            ...
            if self.lambda_edgelen > 0:
                if _mesh is None:
                    _mesh = Meshes(verts=[self.model.vertices + self.model.vertices_offsets], faces=[self.model.triangles])
                loss_edge = mesh_edge_loss(_mesh)
                loss = loss + self.lambda_edgelen * loss_edge

            # L1 reg on vertices offsets
            if self.lambda_offsets > 0:
                ...
                loss = loss + self.lambda_offsets * loss_offsets
