...
class NeRFGUI:
    def __init__(self,
                ...
                train_loader=None,
                debug=True,
                W: int = 1000,  # GUI width
                H: int = 1000, # GUI height
                radius: float = 5,  # default GUI camera radius from center
                fovy: float = 50,  # default GUI camera fovy
                max_spp: int = 1,  # GUI rendering max sample per pixel
                test: bool = False,
                dt_gamma: float = 1/256,
                max_steps: int = 1024,
                bound: int = 2,
                ):
        self.W = W
        ...
        self.dynamic_resolution = True if self.stage == 0 else False
        self.downscale = 1
        self.train_steps = 16
    ...
    def register_dpg(self):
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                dpg.add_slider_float(label="dt_gamma", min_value=0, max_value=0.1, format="%.5f", default_value=self.dt_gamma, callback=callback_set_dt_gamma)

                # max_steps slider
                def callback_set_max_steps(sender, app_data):
                    self.max_steps = app_data
                    self.need_update = True

                dpg.add_slider_int(label="max steps", min_value=1, max_value=1024, format="%d", default_value=self.max_steps, callback=callback_set_max_steps)
                ...
                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="x", width=150, min_value=-self.bound, max_value=0, format="%.2f", default_value=-self.bound, callback=callback_set_aabb, user_data=0)
                    dpg.add_slider_float(label="", width=150, min_value=0, max_value=self.bound, format="%.2f", default_value=self.bound, callback=callback_set_aabb, user_data=3)

                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="y", width=150, min_value=-self.bound, max_value=0, format="%.2f", default_value=-self.bound, callback=callback_set_aabb, user_data=1)
                    dpg.add_slider_float(label="", width=150, min_value=0, max_value=self.bound, format="%.2f", default_value=self.bound, callback=callback_set_aabb, user_data=4)

                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="z", width=150, min_value=-self.bound, max_value=0, format="%.2f", default_value=-self.bound, callback=callback_set_aabb, user_data=2)
                    dpg.add_slider_float(label="", width=150, min_value=0, max_value=self.bound, format="%.2f", default_value=self.bound, callback=callback_set_aabb, user_data=5)
