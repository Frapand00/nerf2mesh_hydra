...
class ColmapDataset:
    def __init__(self,
                ...
                scale: int = 1, # camera radius scale to make sure camera are inside the bounding box.
                offset: List = [0, 0, 0], # camera offset
                fp16: bool = False, # if preload, load into fp16.
                path: str = "", # contains "colmap_sparse"
                enable_cam_center: bool = False,
                bound: int = 2,
                enable_sparse_depth: bool = False,
                enable_dense_depth: bool = False,
                camera_traj: str = "",
                vis_pose: bool = False,
                min_near: float = 0.05,
                stage: int = 0,
                num_rays: int = 4096,
                random_image_batch: bool = False,
                patch_size: int = 1,
                enable_cam_near_far: bool = False,
                ):
        ...
        self.scale = scale # camera radius scale to make sure camera are inside the bounding box.
        # self.offset = offset # camera offset
        self.fp16 = fp16 # if preload, load into fp16.
        self.root_path = path # contains "colmap_sparse"

        self.training = self.type in ['train', 'all', 'trainval']
        self.enable_cam_center = enable_cam_center
        self.bound = bound
        self.enable_sparse_depth = enable_sparse_depth
        self.enable_dense_depth = enable_dense_depth
        self.camera_traj = camera_traj
        self.vis_pose = vis_pose
        self.min_near = min_near
        self.stage = stage
        self.num_rays = num_rays
        self.random_image_batch = random_image_batch
        self.patch_size = patch_size
        self.enable_cam_near_far = enable_cam_near_far
        ...
        # center pose
        self.poses, self.pts3d = center_poses(poses, pts3d, self.enable_cam_center)
        ...
        if np.abs(self.pts_aabb).max() > self.bound:
            print(f'[WARN] ColmapDataset: estimated AABB {self.pts_aabb.tolist()} exceeds provided bound {self.bound}! Consider improving --bound to make scene included in trainable region.')
        ...
        if self.type != 'test':
            ...
            self.sparse_depth_info = [] if self.enable_sparse_depth else None
            self.dense_depth_info = [] if self.enable_dense_depth else None
        ...
        # view all poses.
        if self.vis_pose:
            visualize_poses(self.poses, bound=self.bound, points=self.pts3d)
    ...
    def collate(self, index):
        ...
        if self.enable_sparse_depth and self.stage == 0 and \
            random.random() > 0.9:
            ...
            if self.training and self.stage == 0:
                # randomly sample over images too
                num_rays = self.num_rays

                if self.random_image_batch:
            ...
            rays = get_rays(poses, intrinsics, self.H, self.W, num_rays, patch_size=self.patch_size)
        ...
        if self.enable_cam_near_far and self.cam_near_far is not None:
