...
class Trainer(object):
    def __init__(self,
        ...
        self.lambda_specular = lambda_specular
        self.lambda_lap =lambda_lap
        self.lambda_normal = lambda_normal
        self.lambda_edgelen = lambda_edgelen
        self.lambda_offsets = lambda_offsets
        self.bound = bound
        self.lambda_tv = lambda_tv
        self.adaptive_num_rays = adaptive_num_rays
        self.num_rays = num_rays
        self.num_points = num_points
        self.mark_untrained = mark_untrained
        self.update_extra_interval = update_extra_interval
        self.refine_steps = refine_steps
        print(type(self.num_points))
        ...
        if self.stage > 0 and self.lambda_lpips > 0:
            import lpips
    ...
    def train_step(self, data):
        ...
        # Extract input data
        rays_o = data['rays_o']  # Origin of each ray [N, 3]
        rays_d = data['rays_d']  # Direction of each ray [N, 3]
        index = data['index']  # Index of the camera that generated each ray [1/N]
        cam_near_far = data[
            'cam_near_far'] if 'cam_near_far' in data else None  # Near and far clipping planes of the camera [1/N, 2] or None

        images = data['images']  # Target images to render [N, 3/4]

        N, C = images.shape

        # Convert images from sRGB to linear color space if necessary
        ...
        # Create ground truth RGB values
        if C == 4:
        ...
        # Choose shading mode
        if self.stage == 0 and self.global_step < self.diffuse_step:
            shading = 'diffuse'
        else:
            shading = 'full'

        # Stage 0: Render and compute loss
        if self.stage == 0:

            # Render image using the neural renderer
            ...
            # Compute MSE loss between predicted and ground truth RGB values
            ...
            if 'depth' in data:
                ...
                # Empirically estimate a "local" scale and bias for gt_depth by least squares
                # This is based on the monosdf paper
                # with torch.no_grad():
                ...
                # Compute depth loss
                lambda_depth = self.lambda_depth * min(1.0, self.global_step / 1000)
                mask = gt_depth > 0

                loss_depth = depth_weight * self.criterion(pred_depth * mask, gt_depth * mask)  # [N]
                loss = loss + lambda_depth * loss_depth

        # Stage 1: Render and compute loss
        else:

            # Extract input data for stage 1
            mvp = data['mvp'].squeeze(0)  #

            # Extract H and W from the data dictionary
            H, W = data['H'], data['W']

            # Use the neural network model to render the image for the given rays
            outputs = self.model.render_stage1(rays_o, rays_d, mvp, H, W, bg_color=bg_color, shading=shading)

            # Extract the predicted RGB image and compute the mean loss
            pred_rgb = outputs['image']
            loss = self.criterion(pred_rgb, gt_rgb).mean(-1)  # [H, W]

            # If we are in the refine stage, update triangle errors based on the computed loss
            if self.refine:
            ...
            # If we are in stage 0 compute extra loss:
            if self.stage == 0:

                # If the lambda_entropy hyperparameter is greater than 0, compute the entropy of the weights and weights_sum tensors
                if self.lambda_entropy > 0:
                    ...
                    loss = loss + self.lambda_entropy * (entropy.mean() + entropy2.mean())

                # If the lambda_specular hyperparameter is greater than 0, compute the specular loss
                if self.lambda_specular > 0:
                    specs = outputs['speculars']  # [N, 3] in [0, 1]
                    if specs is not None:
                        loss = loss + self.lambda_specular * (specs ** 2).sum(-1).mean()

            # If we are in stage 1:
            if self.stage == 1:

                # If the lambda_lpips hyperparameter is greater than 0, compute the perceptual loss using the LPIPS criterion
                if self.lambda_lpips > 0:
                    loss = loss + self.lambda_lpips * self.criterion_lpips(
                        pred_rgb.view(1, H, W, 3).permute(0, 3, 1, 2).contiguous(),
                        gt_rgb.view(1, H, W, 3).permute(0, 3, 1, 2).contiguous())

                # If the lambda_lap hyperparameter is greater than 0, compute the smoothness loss using the Laplacian operator
                _mesh = None
                if self.lambda_lap > 0:
                    ...
                    loss = loss + self.lambda_lap * loss_lap

                # If the lambda_normal hyperparameter is greater than 0, compute the normal consistency loss
        ...
        # adaptive num_rays
        if self.stage == 0 and self.adaptive_num_rays:
            self.num_rays = int(round((self.num_points / outputs['num_points']) * self.num_rays))
    ...
    def eval_step(self, data):
        ...
        if self.stage > 0:
            ...
            outputs = self.model.render_stage1(rays_o, rays_d, mvp, H, W, index=index, bg_color=bg_color,)
        else:    
            outputs = self.model.render(rays_o, rays_d, index=index, staged=True, bg_color=bg_color, perturb=False, cam_near_far=cam_near_far)
    ...
    def test_step(self, data, bg_color=None, perturb=False, shading='full'):  
        ...
        if self.stage > 0:
            mvp = data['mvp'].squeeze(0) # [4, 4]
            outputs = self.model.render_stage1(rays_o, rays_d, mvp, H, W, index=index, bg_color=bg_color, shading=shading,)
        else:
            outputs = self.model.render(rays_o, rays_d, index=index, staged=True, bg_color=bg_color, perturb=perturb, cam_near_far=cam_near_far, shading=shading,)
    ...
    def train_gui(self, train_loader, step=16):
        ...
        # mark untrained grid
        if self.global_step == 0 and self.mark_untrained:
            self.model.mark_untrained_grid(train_loader._data)
        ...
        for _ in range(step):
            ...
            if self.model.cuda_ray and self.global_step % self.update_extra_interval == 0:
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if self.stage == 1 and self.refine and self.global_step in self.refine_steps:
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False):
        ...
        if checkpoint is None: # load latest
            checkpoint_list = sorted(glob.glob(f'{self.ckpt_path}/*_stage{self.stage}*.pth'))
        ...
        # if checkpoint_dict['stage'] != self.stage:
        #     return
