...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        self.trainable_density_grid = trainable_density_grid
        self.stage = stage
        self.gui = gui
        ...
        self.refine_decimate_ratio = refine_decimate_ratio
        self.refine_size = refine_size
        self.refine_remesh_size = refine_remesh_size
        self.fp16 = fp16
        self.ssaa = ssaa
        self.visibility_mask_dilation = visibility_mask_dilation
        self.clean_min_f = clean_min_f
        self.clean_min_d = clean_min_d
        self.env_reso = env_reso
        ...
        # density grid
        if not self.trainable_density_grid:
        ...
        if self.stage == 1:
            ...
            for cas in range(self.cascade):
                ...
                _updated_mesh_path = os.path.join(self.workspace, 'mesh_stage0', f'mesh_{cas}_updated.ply') if self.mesh == '' else self.mesh
                if os.path.exists(_updated_mesh_path) and self.ckpt != 'scratch':
                    mesh = trimesh.load(_updated_mesh_path, force='mesh', skip_material=True, process=False)
                else: # base (not updated)
                    mesh = trimesh.load(os.path.join(self.workspace, 'mesh_stage0', f'mesh_{cas}.ply'), force='mesh', skip_material=True, process=False)
    ...
    def get_params(self, lr):
        ...
        if self.individual_codes is not None:
            params.append({'params': self.individual_codes, 'lr': self.lr * 0.1, 'weight_decay': 0})

        if self.trainable_density_grid:
            params.append({'params': self.density_grid, 'lr': self.lr, 'weight_decay': 0})

        if self.glctx is not None:
            params.append({'params': self.vertices_offsets, 'lr': self.lr_vert, 'weight_decay': 0})
    ...
    def refine_and_decimate(self):
        ...
        if self.bound <= 1:
            ...
            # mesh.export(os.path.join(self.workspace, 'mesh_stage0', 'mesh_0_before_updated.ply'))

            v, f = decimate_and_refine_mesh(v, f, mask, decimate_ratio=self.refine_decimate_ratio, refine_size=self.refine_size, refine_remesh_size=self.refine_remesh_size)
            # export
            mesh = trimesh.Trimesh(v, f, process=False)
            mesh.export(os.path.join(self.workspace, 'mesh_stage0', 'mesh_0_updated.ply'))
            ...
            for cas in range(self.cascade):
                ...
                if cas == 0:
                    cur_v, cur_f = decimate_and_refine_mesh(cur_v, cur_f, mask, decimate_ratio=self.refine_decimate_ratio, refine_size=self.refine_size, refine_remesh_size=self.refine_remesh_size)

                mesh = trimesh.Trimesh(cur_v, cur_f, process=False)
                mesh.export(os.path.join(self.workspace, 'mesh_stage0', f'mesh_{cas}_updated.ply'))
    ...
    def export_stage1(self, path, h0=2048, w0=2048, png_compression_level=3):
        ...
        for cas in range(self.cascade):
            cur_v = v[self.v_cumsum[cas]:self.v_cumsum[cas+1]]
            cur_f = f[self.f_cumsum[cas]:self.f_cumsum[cas+1]] - self.v_cumsum[cas]
            _export_obj(cur_v, cur_f, h0, w0, self.ssaa, cas)
    ...
    def export_stage0(self, save_path, resolution=None, decimate_target=1e5, dataset=None, S=128):
        ...
        if dataset is not None:
            ...
            vertices, triangles = remove_masked_trigs(vertices, triangles, visibility_mask, dilation=self.visibility_mask_dilation)

        ### reduce floaters by post-processing...
        vertices, triangles = clean_mesh(vertices, triangles, min_f=self.clean_min_f, min_d=self.clean_min_d, repair=True, remesh=False)
        ...
        if self.bound > 1:
            ...
            for cas in range(1, self.cascade):
                ...
                # clean mesh
                vertices_out, triangles_out = clean_mesh(vertices_out, triangles_out, min_f=self.clean_min_f, min_d=self.clean_min_d, repair=False, remesh=False)
                ...
                if dataset is not None:
                    ...
                    vertices_out, triangles_out = remove_masked_trigs(vertices_out, triangles_out, visibility_mask, dilation=self.visibility_mask_dilation)
    ...
    def render(self, rays_o, rays_d, index=None, dt_gamma=0, bg_color=None, perturb=False, max_steps=1024, T_thresh=1e-4, cam_near_far=None, shading='full', **kwargs):
        ...
        if self.training:
            ...
            xyzs, dirs, ts, rays = raymarching.march_rays_train(rays_o, rays_d, self.real_bound, self.contract, self.density_bitfield, self.cascade, self.grid_size, nears, fars, perturb, dt_gamma, max_steps)
            ...
            while step < max_steps:
                ...
                xyzs, dirs, ts = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, self.real_bound, self.contract, self.density_bitfield, self.cascade, self.grid_size, nears, fars, perturb if step == 0 else False, dt_gamma, max_steps)
    ...
    def render_stage1(self, rays_o, rays_d, mvp, h0, w0, index=None, bg_color=None, shading='full', **kwargs):
        ...
        if mask_flatten.any():
            with torch.cuda.amp.autocast(enabled=self.fp16):
                mask_rgbs, masked_specular = self.rgb(
                    xyzs[mask_flatten] if self.enable_offset_nerf_grad else xyzs[mask_flatten].detach(),
        ...
        alphas = dr.antialias(alphas, rast, vertices_clip, self.triangles, pos_gradient_boost=self.pos_gradient_boost).squeeze(0).clamp(0, 1)
        rgbs = dr.antialias(rgbs, rast, vertices_clip, self.triangles, pos_gradient_boost=self.pos_gradient_boost).squeeze(0).clamp(0, 1)
                ...
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            min_near = self.min_near if cam_near_far is None else cam_near_far[head:tail, 0].unsqueeze(1)
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        if not self.trainable_density_grid:
            ...
            if self.lambda_density > 0:
                for cas in range(1, self.cascade):
                    loss_density = loss_density + (2 ** (cas - 1)) * self.lambda_density * self.density_grid[cas][valid_mask[cas]].mean()
